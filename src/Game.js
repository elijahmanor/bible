...
export default function Game({ verse, gameIndex, answerType, onComplete }) {
    ...
    const typing = useRef();
    const words = verse.text.trim().split(" ").filter(w => w);
    ...
    useEffect(() => {
      ...
      let answer = wordIndex < words.length - 1 ? words[wordIndex].replace(/\W/g, "") : "";
      if (
        gameIndex === 3 &&
    ...
    const handleInput = e => {
      ...
      if (gameIndex === 1 || gameIndex === 2 || gameIndex === 5) {
